...
checkNpmVersions({ '@activitree/simpl-schema': '>=0.0.0' }, 'aldeed:collection3')

const SimpleSchema = require('@activitree/simpl-schema').default

// Exported only for listening to events
const Collection3 = new EventEmitter()

Collection3.cleanOptions = {
  filter: true,
  autoConvert: true,
...
Mongo.Collection.prototype.attachSchema = function c2AttachSchema (ss, options) {
  ...
  Collection3.emit('schema.attached', this, ss, options)
}
/**
 * simpleSchema
...
attachSimpleSchema(Mongo.Collection)
attachSimpleSchema(LocalCollection)

// Wrap DB write operation methods
const doWrapDB = methodName => {
...
function doValidate (collection, type, args, getAutoValues, userId, isFromTrustedCode) {
  ...
  const isUpsert = (type === 'update' && options.upsert === true)
  ...
  const properties = ['autoConvert', 'filter', 'removeEmptyStrings', 'removeNullsFromArrays', 'trimStrings']
  for (const prop in properties) {
    if (typeof options[properties[prop]] === 'boolean') {
  ...
  schema.clean(doc, {
    ...
    ...Collection3.cleanOptions,
    // The extended with the schema-level defaults (from SimpleSchema constructor options)
